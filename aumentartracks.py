...
def comparar_valores(datos):
    combined_data = (datos[0] << 24) | (datos[1] << 16) | (datos[2] << 8) | datos[3]  # Combinar los cuatro valores en uno solo
    ...
    if combined_data <= 0x3FC00000:
        ...
        return 0, 0  # Valor fuera del rango definido
...
def transmit_selected_bytes():
    ...
    if len(second_part_buffer) >= 339:
        # Obtener los datos de la segunda parte para comparar
        datos_comparar = second_part_buffer[28:32]
        datos_comparar2 = second_part_buffer[48:52]
        # Realizar la comparación con los valores obtenidos
        decimal_value, decimalmsb_value = comparar_valores(datos_comparar)
        decimal_value2, decimalmsb_value2 = comparar_valores(datos_comparar2)       
        selected_bytes1 = [
        ...
        uart_out.write(bytearray(selected_bytes1))
        print("Bytes seleccionados transmitidos:", selected_bytes1)
        uart_out.write(bytearray(selected_bytes2))
        print("Bytes seleccionados transmitidos:", selected_bytes2)
        # Limpiar los buffers para el próximo mensaje
